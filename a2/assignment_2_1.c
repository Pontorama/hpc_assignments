...
#include <x86intrin.h>
static inline 
unsigned short get_distance(short *restrict point_1,
...
static inline 
short convert_to_short(char a[2], char b[3]) {
...
//Assigns to double pointer for reduction between threads
  ...
  for (size_t i = 0; i < n_threads; i++) {
    distances[i] = distance_entries + i * max_distance;
 	...
 	const unsigned int max_chunk = 600000;
  unsigned int chunk_size;
  // Case: Whole file read
  ...
  char *moving_string =
	...
	//format for reading a certain number of characters from file
...
#pragma omp parallel shared(                                                   \
        base, base_entries, distances, chunk_string,       \
             format) private(distance_one, distance_two,            \
...
#pragma omp for
	...
	#pragma omp for
		      ...
		      unsigned int rest_size = file_size % chunk_size;
					//In case the number of lines are not divisble by chunk_size
					if (rest_size != 0){
	        ...
	        for (unsigned int i = 0; i < chunk_size; i++) {
	#pragma omp for
	          for (unsigned int j = 0; j < chunk_size - 3; j += 4) {
	            distance_one = get_distance(base[i], moving[j]) / 10;
	            distance_two = get_distance(base[i], moving[j + 1]) / 10;
	            distance_three = get_distance(base[i], moving[j + 2]) / 10;
	            distance_four = get_distance(base[i], moving[j + 3]) / 10;
